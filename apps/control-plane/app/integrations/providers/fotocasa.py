"""
Fotocasa Integration Provider (Spain)
"""
...
async def properties_list(
    ...
    """List properties from Fotocasa."""
    if connection.status.value != "connected":
        raise ValueError("Fotocasa not connected")
    ...
    try:
        ...
        result = await nango.proxy(
            provider=Provider.FOTOCASA.value,
        ...
        log_operation(tenant_id, Provider.FOTOCASA, "properties_list", params, result)
        return result.get("properties", [])
    
    except Exception as e:
        log_operation(tenant_id, Provider.FOTOCASA, "properties_list", params, error=str(e))
        raise
...
async def property_create(
    ...
    if requires_approval(operation_name):
        preview = {
            "action": "Create Fotocasa Property",
            "title": title,
            "address": address,
            "price": price,
            "operation": operation
    ...
    try:
        ...
        log_operation(tenant_id, Provider.FOTOCASA, operation_name, parameters, result)
        return result.get("property", {})
    
    except Exception as e:
        log_operation(tenant_id, Provider.FOTOCASA, operation_name, parameters, error=str(e))
        raise
